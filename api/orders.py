...
async def create_inventory_type(
    ...
    """
    Crea un nuevo tipo de inventario para la empresa actual. La posición se asigna automáticamente.
    """
    # Calcular la siguiente posición
    max_position_stmt = select(func.max(InventoryTypes.position)).where(
        InventoryTypes.company_id == current_user.company_id
    )
...
async def get_all_inventory_types(
    ...
    """
    Obtiene todos los tipos de inventario de la empresa actual.
    """
    stmt = select(InventoryTypes).where(
        InventoryTypes.company_id == current_user.company_id
    ).order_by(InventoryTypes.position)
...
async def update_inventory_type(
    ...
    """
    inventory_type = db.scalar(
        select(InventoryTypes).where(
            InventoryTypes.inv_type_id == inv_type_id,
            InventoryTypes.company_id == current_user.company_id
        )
    )
...
@router.delete("/inventory-types/{inv_type_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_inventory_type(
    ...
    """
    Elimina un tipo de inventario y sus ítems asociados si pertenecen a la empresa actual.
    """
    inventory_type = db.scalar(
        select(InventoryTypes).where(
            InventoryTypes.inv_type_id == inv_type_id,
...
async def reorder_inventory_types(
    ...
    """
    Reordena una lista de tipos de inventario de la empresa actual.
    """
    if not reorder_data:
    ...
    try:
        types_to_update = db.scalars(
            select(InventoryTypes).where(
                InventoryTypes.inv_type_id.in_(inv_type_ids),
                InventoryTypes.company_id == current_user.company_id
            )
        ).all()
...
async def create_inventory_item(
    ...
    """
    Crea un nuevo ítem de inventario para la empresa actual.
    """
    # Validar que el tipo de inventario pertenece a la empresa actual
    inv_type = db.scalar(
        select(InventoryTypes).where(
            InventoryTypes.inv_type_id == inventory_item.inv_type_id,
            InventoryTypes.company_id == current_user.company_id
        )
    )
    if not inv_type:
    ...
    max_position_stmt = select(func.max(InventoryItems.position)).where(
        ...
        InventoryItems.company_id == current_user.company_id
    )
    max_position = db.scalar(max_position_stmt)
    next_position = (max_position + 1) if max_position is not None else 0
...
async def get_inventory_items_by_type(
    ...
    """
    Obtiene un tipo de inventario y todos sus ítems asociados de la empresa actual.
    """
    inventory_type = db.scalar(
        select(InventoryTypes).where(
            InventoryTypes.inv_type_id == inv_type_id,
    ...
    stmt = select(InventoryItems).where(
        InventoryItems.inv_type_id == inv_type_id,
        InventoryItems.company_id == current_user.company_id
    ).order_by(InventoryItems.position)
    items = db.scalars(stmt).all()
...
async def delete_inventory_item(
    ...
    """
    Elimina un ítem de inventario si pertenece a la empresa actual.
    """
    item = db.scalar(
...
async def reorder_inventory_items(
    ...
    """
    Reordena una lista de ítems de inventario de la empresa actual.
    """
    if not reorder_data:
    ...
    try:
        items_to_update = db.scalars(
            select(InventoryItems).where(
                InventoryItems.item_id.in_(item_ids),
                InventoryItems.company_id == current_user.company_id
            )
        ).all()
...
async def update_inventory_items(
    ...
    """
    Actualiza parcialmente múltiples ítems de inventario en una sola operación para la empresa actual.
    """
    ...
    try:
        ...
        if len(items_in_db) != len(item_ids):
            ...
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"No se encontraron los siguientes ítems o acceso denegado: {list(missing_ids)}"
            )
...
async def upsert_order_inventory_data(
    ...
    """
    Crea o actualiza (Upsert) múltiples entradas de datos de inventario para una orden de la empresa actual.
    """
    if not data_entries:
        return []
    ...
    # Validar que la orden pertenece a la empresa actual
    order = db.scalar(
        select(Order).where(Order.order_id == order_id, Order.company_id == current_user.company_id)
    )
    if not order:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Orden no encontrada o acceso denegado.")
    ...
    for entry_data in data_entries:
        # Validar que el item de inventario pertenece a la misma empresa
        item = db.scalar(
            select(InventoryItems).where(
                InventoryItems.item_id == entry_data.item_id,
                InventoryItems.company_id == current_user.company_id
            )
        )
        if not item:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Ítem de inventario {entry_data.item_id} no encontrado o acceso denegado.")
...
async def get_order_inventory_data(
    ...
    """
    Obtiene todas las entradas de datos de inventario para una orden y tipo de inventario específicos de la empresa actual.
    """
    # Validar que la orden pertenece a la empresa actual
    order = db.scalar(
        select(Order).where(Order.order_id == order_id, Order.company_id == current_user.company_id)
    ...
    # Obtener los item_ids que pertenecen al inv_type_id y a la empresa actual
    item_ids_stmt = select(InventoryItems.item_id).where(
        InventoryItems.inv_type_id == inv_type_id,
        InventoryItems.company_id == current_user.company_id
    )
